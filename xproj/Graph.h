...
public:
    ...
    Vertex get_island(int) const;//Returns island at position

    Vertex connect(Vertex, string); //finds an island a bridge connects to
    void print_matrix(ostream &) const; //reports the edges of the graph
    void map_of_konisberg() const; //prints map of konisberg
    int find_index(Vertex v); //finds index of vertice

    // Helpers functions for implementing BFS and DFS
    bool is_vertex(Vertex) const;                        //checks if exists
    bool is_edge(string, int);    //checks if a bridge is in the graph
    void get_to_vertices(Vertex, Queue<Vertex> &) const; //enqueues neighbors
    void neighbor_stack(Vertex, Stack<Vertex> &) const; //pushes neighor to stack
    ...
    void bridge_finder(int) const; //finds bridge with only taking one arg
