...
{
    find_pirate(p, true); // going to use find_pirate function to see if the pirate passed is in the list. 
    //finish with if statement
    pirates[length] = p;
    length++;
}
...
void ArrayList::delete_pirate(Pirate p)
{
    ...
    {
        if (p == pirates[location]) // find the pirate 
            //this is where i delete the pirate. set it to null? 
            break;
        else
            location++;
    }
